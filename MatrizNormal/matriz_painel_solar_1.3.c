#include <stdio.h>
#include <string.h>
#define TAM 2 //Definição do tamanho das colunas e linhas das matrizes.
...
typedef Painel matriz_solar[TAM][TAM];
...
void Ler_Mat(matriz_solar a) {
	...
	for(i = 0; i < TAM; i++){
		for(j = 0; j < TAM; j++){
			...
			scanf("%d", &a[i][j].ativo);
			if(a[i][j].ativo == 1){
                printf("\nEntre com a geracao em Watts do painel [%d][%d]: ", i + 1, j + 1);
                scanf("%f", &a[i][j].geracao);
                printf("\nEntre com o tempo de atividade em horas do painel [%d][%d]: ", i + 1, j + 1);
                scanf("%f", &a[i][j].atividade);
                a[i][j].eficiencia = ((a[i][j].geracao / a[i][j].atividade) / CAP) * 100;
                printf("\nA eficiencia do painel [%d][%d] e: %.2f%%", i + 1, j + 1, a[i][j].eficiencia);
                printf("\nEntre com a inclinacao do painel [%d][%d]: ", i + 1, j + 1);
                scanf("%f", &a[i][j].inclinacao);
			}
			else{
                a[i][j].geracao = 0;
                a[i][j].atividade = 0;
...
void Exibir_Mat(matriz_solar a) {
    ...
    for(i = 0; i < TAM; i++){
        ...
        for(j = 0; j < TAM; j++){
            ...
            printf("Ger.: %.2f W", a[i][j].geracao );
            if(a[i][j].geracao > 9999 || a[i][j].geracao < -999) printf("\t");
            else printf("\t\t");
        }
        printf("\n");
            ...
            printf("Ativ.: %.2f h", a[i][j].atividade );
            if(a[i][j].atividade > 99 || a[i][j].atividade < -99) printf("\t");
            else printf("\t\t");
        }
        printf("\n");
        ...
        for(j = 0; j < TAM; j++){
            ...
            printf("Efic.: %.2f%%", a[i][j].eficiencia );
            if(a[i][j].eficiencia > 9999 || a[i][j].eficiencia < -999) printf("\t");
            else printf("\t\t");
        ...
        for(j = 0; j < TAM; j++){
            ...
            printf("Incl.: %.2f", a[i][j].inclinacao );
            if(a[i][j].inclinacao > 9999 || a[i][j].inclinacao < -99) printf("\t");
            else printf("\t\t");
        }
        printf("\n\n");
    }
...
void Buscar_Elemento_Mat(int f, int i, int j, matriz_solar a) {
	switch(f){
	case 0:
		printf("\n\n%d", a[i - 1][j - 1].ativo); //ativo
		break;

	case 1:
		printf("\n\n%.2f W", a[i - 1][j - 1].geracao); //geracao
		break;

	case 2:
		printf("\n\n%.2f h", a[i - 1][j - 1].atividade); //atividade
		break;

	case 3:
		printf("\n\n%.2f%%", a[i - 1][j - 1].eficiencia); //eficiencia
		break;

	case 4:
		printf("\n\n%.2f", a[i - 1][j - 1].inclinacao); //inclinacao
		break;
...
int main ( ) {
	...
	while (index != 0 && erros < LIMITE){
		...
		getchar();  //impede saída imediata da tela
		getchar();
